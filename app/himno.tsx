...
import { opacityColor } from "@/res/helper";
...
const HimnoSongScreen = (props: { route: any; navigation: any }) => {
  ...
  const _himno = JSON.parse(router.params.himnoStr as string);
  const [isFavorite, setIsFavorite] = useState(false);
  const [himno, setHimno] = useState(_himno);
  ...
  const verses = paragraphs?.map((item: any, i: number) => {
    ...
    let filter;
    if (chorus !== undefined) {
      filter = chorus.filter((choirItem: { chorus_position_ignore: any }) => compareArrayIgnore(choirItem.chorus_position_ignore, i + 1));
  ...
  function joinChoirs(filter: any[]) {
    return filter.length >= 2
      ? filter.reduce(
          (accumulatorChoir: any, currentChoir: { choir: any }, currentIndex: number) => accumulatorChoir + currentChoir.choir + (filter.length !== currentIndex + 1 ? "\n\n" : ""),
          ""
        )
      : filter[0].choir;
  }
    ...
    navigation?.setOptions({
      ...
      headerRight: () => (
        <View style={styles.headerRightContainer}>
          <Button color={Colors.bkgTransparentPrimary} onPress={() => onPressFontSize(-initialValues.fontSizeIncremental)} title="-T" />
          <Button color={Colors.bkgTransparentPrimary} onPress={() => onPressFontSize(initialValues.fontSizeIncremental)} title="+T" />
        </View>
      ),
    });
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList
        ...
        renderItem={({ item, index }) => <ItemHimnoLetter item={item} isFinalVerse={verses.length - 1 === index} customFontSize={customFontSize} />}
      />

      <Pressable onPress={() => toggleFavorite()} style={[styles.containerFloat, isFavorite && styles.containerFloatFavorite]}>
